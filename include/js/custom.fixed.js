      ...
      $('.new-post hr').each(function () {
        ...
        gsap.to($(this), {
          ...
          scrollTrigger: {
            ...
            toggleActions: 'play none none none', // Play the animation on the enter, no actions on leave, enter back, or leave back
          },
        })
      })
    })
...
// Route-detection: open contact section when user visits /contact, #contact or ?section=contact
;(function () {
  function openContact() {
    var navLink = document.querySelector('[data-target="contact-section"], a[href="#nav-contact"]')
    if (navLink) {
      navLink.click()
      return
    }
    ...
    if (section) {
      ...
      var firstInput = section.querySelector('input, textarea, [tabindex]')
      if (firstInput) try { firstInput.focus({preventScroll: true}) } catch (e) { /* ignore */ }
    }
  }

  function shouldOpenContact() {
    var p = (window.location.pathname || '').replace(/\/+/g, '/').replace(/\/$/, '')
    var q = new URLSearchParams(window.location.search || '')
    var h = (window.location.hash || '').replace('#', '')

    if (p.split('/').indexOf('contact') !== -1) return true
    if (h === 'contact' || h === 'nav-contact' || h === 'contact-section') return true
    if (q.get('section') === 'contact' || q.get('open') === 'contact') return true
    return false
  }

  if (document.readyState === 'complete' || document.readyState === 'interactive') {
    setTimeout(function () { if (shouldOpenContact()) openContact() }, 120)
  } else {
